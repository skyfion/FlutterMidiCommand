...
class FlutterMidiCommandPlugin : FlutterPlugin, ActivityAware, MethodCallHandler,
    ...
    private lateinit var usbMidiManager: UsbMidiManager
...
// Clean up references.
    ...
    companion object {
        ...
        fun registerWith(registrar: Registrar) {
            val instance = FlutterMidiCommandPlugin()
    ...
    fun setup() {
        print("setup")
        Log.d("FlutterMidiCommand", "setup!!!")
        ...
        usbMidiManager = UsbMidiManager(context)
        { this.connectedDevices.remove(it.id) } // when remove device

        usbMidiManager.open()

        rxStreamHandler = FMCStreamHandler(handler)
        rxChannel =
            EventChannel(messenger, "plugins.invisiblewrench.com/flutter_midi_command/rx_channel")
        rxChannel.setStreamHandler(rxStreamHandler)

        setupStreamHandler = FMCStreamHandler(handler)
        setupChannel = EventChannel(
            messenger,
...
//    Log.d("FlutterMIDICommand","call method ${call.method}")
        ...
        when (call.method) {
            "sendData" -> {
                ...
                sendData(
                    args?.get("data") as ByteArray,
                    args["timestamp"] as? Long,
...
//        }
    ...
    fun startVirtualService() {
        val comp =
            ComponentName(context, "com.invisiblewrench.fluttermidicommand.VirtualDeviceService")
...
//        }
    ...
    private fun connectToDevice(deviceId: String, type: String): String? {
        ...
        } else if (type == "usbNative") {
            val device = usbMidiManager.getDevices().find { d -> d.id == deviceId }
            if (device != null) {
...
//              SCAN_FAILED_SCANNING_TOO_FREQUENTLY to "Scanning too frequently"
    ...
    fun listOfDevices(): List<Map<String, Any>> {
        ...
        connectedGattDevices?.forEach {
            Log.d(
                "FlutterMIDICommand",
                "connectedGattDevice ${it.address} type ${it.type} name ${it.name}"
            )
        ...
        bondedDevices?.forEach {
            Log.d(
                "FlutterMIDICommand",
                "add bonded device ${it.address} type ${it.type} name ${it.name}"
            )
...
//        }
        ...
        Log.d("FlutterMIDICommand", "scan native usb...")
        val usbDevs: HashSet<UsbDeviceAdapter> = usbMidiManager.getDevices()
        usbDevs.forEach {
            val id = it.id
            Log.d("FlutterMIDICommand", "add device from usbMidiManager id $id")

            if (list.contains(id)) {
                Log.d("FlutterMIDICommand", "device already in list $id")
            } else {
                Log.d("FlutterMIDICommand", "add native device $id type usbNative")
                list[id] = mapOf(
                    "name" to it.name,
                    "id" to id,
                    "type" to "usbNative",
    ...
    private val deviceOpenedListener = MidiManager.OnDeviceOpenedListener { it ->
